    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            ZStack(alignment: .bottom) {
                // Bulb
                Bulb()
                    .fill(isSelected ? bulbColor.opacity(0.6) : .streetLightOff.opacity(0.6))
                ...
                // Filament
                Filament()
                    .stroke(isSelected ? bulbColor : .streetLightOff, style: StrokeStyle(lineWidth: 2 * CGFloat(scale)))
                    .frame(width: 15 * CGFloat(scale), height: 25.0 * CGFloat(scale))
                    .glow(color: colorScheme == .dark && isSelected ? Color.brightHouse.opacity(0.1) : .clear, radius: 200)
            }
...
struct IncandescentContainer: View {
    // Handle color scheme
    @Environment(\.colorScheme) var colorScheme
    ...
    var body: some View {
        Button {
            ...
            VStack(spacing: 8) {
                ...
                Incandescent(isSelected: isSelected)
...
struct Incandescent_Previews: PreviewProvider {
    static var previews: some View {
        Incandescent(isSelected: true)
    }
}
