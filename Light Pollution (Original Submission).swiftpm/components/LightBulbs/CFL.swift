...
struct CFL: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            // Bulb
            HStack(spacing: 2 * CGFloat(scale)) {
                VStack(spacing: 0) {
                    HalfCircle()
                        .fill(isSelected ? bulbColor : .streetLightOff)
                        .frame(width: 5 * CGFloat(scale), height: 2.5 * CGFloat(scale))
                    
                    Rectangle()
                        .fill(isSelected ? bulbColor : .streetLightOff)
                        .frame(width: 5 * CGFloat(scale), height: 35 * CGFloat(scale))
                }
...
struct CFL_Previews: PreviewProvider {
